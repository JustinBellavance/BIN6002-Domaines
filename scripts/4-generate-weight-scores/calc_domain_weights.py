# This script serve to calculate domain weights as mentioned in Lee and Lee 2008 article
# ex: python calc_weights.py ref_architectures.tsv
...
def load_architectures(ref_architectures_filepath ,select_name = 1):
	...
	with open(ref_architectures_filepath) as f:
		for l in f:
			architecture_with_id = l[:-1].split("\t")[1].split(",") 	# PF0001:dom1,PF0002:dom2
...
if __name__ == '__main__':
	if len(sys.argv) < 2:
		...
		print("usage: python", sys.argv[0], "ref_architectures.tsv")
	else:
		main(sys.argv[1])
